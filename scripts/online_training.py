...
def batch_wise_datagen(
    ...
    logger.debug(f"Generated matrices A: {A}")
    logger.debug(f"Generated matrices B: {B}")
    logger.debug(f"Generated matrices C: {C}")
...
def train(
    ...
    for i in range(args.num_batches):
        ...
        system_outputs = system_outputs.transpose(1, 2).to(device)
        # outputs, hidden_states = model(hidden_truths, system_outputs)# For RNN
        outputs = model(hidden_truths)  # For RN
        ...
        plot_functions(
            ...
            save_path=f"{args.save_destination}/plot_{i:03d}.png",
        )
