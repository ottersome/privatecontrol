...
def paretto_frontier_up(
    ...
    x_max_idx = np.argmax(cp[:, 0])
    y_max_idx = np.argmax(cp[:, 1])

    excess_down = np.where(cp[:, 1] < cp[x_max_idx, 1])[0]
    excess_left = np.where(cp[:, 0] < cp[y_max_idx, 0])[0]

    # Remove excess points
    if len(excess_down) > 0:
        cp = np.delete(cp, excess_down, axis=0)
    if len(excess_left) > 0:
        cp = np.delete(cp, excess_left, axis=0)

    return cp[:, 0], cp[:,1]


def paretto_frontier_down(
    x: np.ndarray, y: np.ndarray
) -> tuple[np.ndarray, np.ndarray]:
